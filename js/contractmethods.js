   ...
   function initializeContract() {
     ...
     if (typeof window.web3 !== 'undefined') 
        ...
        else 
        {
         ...
         web3.eth.defaultAccount = "0x248c558957b13A494f4c3A6Fb6D63a380eDa526B";
         return new web3.eth.Contract(CandidateABI, "0x3b62df448cbddeb68fc382d16dd91a3cd1ae5084");
  ...
  async function getDetails(){
    ...
    async function displayCandidate() {
        ...
        var cand2 = document.getElementById("nocand");
         ...
         var CandContract = initializeContract();
         accounts = await ethereum.request({ method: 'eth_requestAccounts' });
         CandContract.methods._getValues().call({from : accounts[0]}).then(function(result)
          {
            totalcand = result[0];
         ...
         function display2(tocand)
         {
          ...
          {      
               // calling create function
                   create(i,n);
                   CandContract.methods.cands(i).call({from : accounts[0]}).then(function(result)
                   {     
                 
                         document.getElementById("r"+ n).innerHTML = result[0] + " . " + result[2] + "\n";
                         document.getElementById("r" + (n+1)).innerHTML = result[3];
    ...
    async function addCandidate()
    {
          ...
          var id = parseInt(document.getElementById("ID").value);
            ...
            var CandContract = initializeContract();
            accounts = await ethereum.request({ method: 'eth_requestAccounts' });
            CandContract.methods._createCand(id,name,descr).send({from: accounts[0]}).then(function(err,result)
            {
    ...
    async function uploadPaillier()
    {
          var public = document.getElementById("public").value;
          var CandContract = initializeContract();
          accounts = await ethereum.request({ method: 'eth_requestAccounts' });
            CandContract.methods._getPailKey(public).send({from: accounts[0]}).then(function(result)
            {
                  console.log(result);
                  if(result)
                  {
    ...
    async function uploadSLRSParam()
    {
      var k = document.getElementById('k').value ;
        ...
        var CandContract = initializeContract();
        accounts = await ethereum.request({ method: 'eth_requestAccounts' });
            CandContract.methods._getSLRSparam(k,l,N,e,d,H1).send({from: accounts[0]}).then(function(result)
            {
    ...
    async function createVoter()
    {
      ...
      id = parseInt(id);
       var CandContract = initializeContract();
       accounts = await ethereum.request({ method: 'eth_requestAccounts' });
            CandContract.methods._createVoter(id).send({from: accounts[0]}).then(result =>
            {
    ...
    async function getParameters()
    {
        ...
        var CandContract = initializeContract();
        accounts = await ethereum.request({ method: 'eth_requestAccounts' });
        CandContract.methods._getParams().call({from : accounts[0]}).then(function(result)
        {
          console.log(result);
           if(result)
           {
           document.getElementById('k').value = result[0] ;
           document.getElementById('l').value = result[1] ;
           document.getElementById('n').value = result[2] ;
           document.getElementById('e').value = result[3] ;
           document.getElementById('h1').value = result[4] ;
          }
        });

        CandContract.methods.paillierKey().call({from : accounts[0]}).then(function(result)
        {
    ...
    async function generateSLRSKeys()
    {
          ...
          CandContract.methods.verify(id).call({from: accounts[0]}).then(function(result)
          {
                if(result == true)
                {
                   CandContract.methods.generateKeys(id).call({from : accounts[0]}).then(function(result)
                {
                     console.log(result);
                ...
                {
                   CandContract.methods.generateKeys(id).send({from: accounts[0]}).then(function(result)
                    {
                        console.log(result);
                        if(result)
                        { 
   ...
   async function verify()
    {
         ...
         var id = document.getElementById("klen").value;
          ...
          var CandContract = initializeContract();
          accounts = await ethereum.request({ method: 'eth_requestAccounts' });
            CandContract.methods.uploadPubkey(id,publick).send({from: accounts[0]}).then(function(result)
            {
    ...
    async function uploadEpsilon(Epsilon)
    {
        var CandContract = initializeContract();
        accounts = await ethereum.request({ method: 'eth_requestAccounts' });
        CandContract.methods.totalcand().call({from : accounts[0]}).then(function(result)
        {
                 if(result)
                 {
                    for(let i = 1; i <= result ; i++)
                    {
                       CandContract.methods._getEpsilon(i,Epsilon[i-1]).send({from: accounts[0]}).then(function(result)
                       {
                          if(result)
                          {
                             console.log(result);
    ...
    async function getEpsilon(candidate)
    {
        console.log(candidate);
        var _id = candidate;
        var CandContract = initializeContract();
        accounts = await ethereum.request({ method: 'eth_requestAccounts' });
        CandContract.methods.Epsilonval(_id).call({from : accounts[0]}).then(function(result)
        {
               if(result)
               {
   ...
   async function voterVote(candidate_id)
    {
          ...
          var CandContract = initializeContract();
          accounts = await ethereum.request({ method: 'eth_requestAccounts' });
          CandContract.methods.voterdetail(voterid).call({from : accounts[0]}).then(function(result)
            {
               console.log(result);
               if(result[2]==true)
               {
               ...
               {
                  CandContract.methods._vote(voterid,_id,slrs,epsilon).send({from: accounts[0]}).then(function(result)
                  {
                        if(result)
                        {
                           alert("You have successfully voted!! Thank You");
                           window.location.href = "votVoting.php";
                        }
                  });
               }
           });     
    ...
    function getPie(data)
    {
      ...
      src: "http://localhost/project/images/backpie.jpg",
      mode: "fit"
   });
      // add the data
      chart.data(data);
    ...
    async function calculateWinner()
    {
       ...
       var CandContract = initializeContract();
        accounts = await ethereum.request({ method: 'eth_requestAccounts' });
        CandContract.methods.triggerTally().call({from : accounts[0]}).then(function(result)
        {
            if(result == true)
            {
                CandContract.methods.totalcand().call({from : accounts[0]}).then(function(result)
            ...
            {
               alert("Please Wait for the election to get over.");
            }
        });
  ...
  // function to calculate winner
  async function calculate(counts,total)
  {
        var CandContract = initializeContract();
        accounts = await ethereum.request({ method: 'eth_requestAccounts' });
        console.log(counts);
        ...
        console.log(max);
        console.log(winid);
        CandContract.methods._getWinner(winid).send({from : accounts[0]}).then(function(result)
        {
            if(result)
            {
               alert("Winner successfully uploaded");
    ...
    async function getWinner()
    {
        ...
        CandContract.methods.triggerTally().call({from : accounts[0]}).then(function(result)
        {
            ...
            if(result == true)
            {
                CandContract.methods.displayWinner().call({from : accounts[0]}).then(function(result)
                {        
                    console.log(result);
                     if(result)
                     {
   ...
   async function triggerTally()
   {
       ...
       CandContract.methods.triggerTally().call({from : accounts[0]}).then(function(result)
       {
            if(result == true)
            {
               alert("Tally phase already triggerred");
            }
            else
            {
               CandContract.methods._triggerTallyPhase().send({from : accounts[0]}).then(function(result)
              {        
                console.log(result);
                 if(result)
                 {
   ...
   async function checkVerified(votid)
   {
       var id = votid;
       console.log(votid);
       var CandContract = initializeContract();
       accounts = await ethereum.request({ method: 'eth_requestAccounts' });
        CandContract.methods.verify(id).call().then(function(result)
        {        
                console.log(result);
                 if(result == false)
                 {
                    alert("Please proceed to the Blockchain page for authentication.");
                 }
        });
   }
